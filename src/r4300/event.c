...
{
	if (!bInitialized) Event_init();
	if (mt_o.bEventDMsg) printf("%lu   Event_ReceiveAll(0x%08X) wait\n", pthread_self(), Flags);
	pthread_mutex_lock(&EventLock);
	...
	Flag &= ~Flags;
	pthread_mutex_unlock(&EventLock);
	if (mt_o.bEventDMsg) printf("%lu   Event_ReceiveAll(0x%08X) received\n", pthread_self(), Flags);
	return 0;
}
...
{
	if (!bInitialized) Event_init();
	if (mt_o.bEventDMsg) printf("%lu   Event_ReceiveAny(0x%08X) wait\n", pthread_self(), Flags);
	pthread_mutex_lock(&EventLock);
	...
	pthread_mutex_unlock(&EventLock);
	if (mt_o.bEventDMsg) printf("\t%lu   Event_ReceiveAny(0x%08X) received \n", pthread_self(), *Found);
	return 0;
}
